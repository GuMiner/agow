...
bool SignGenerator::LoadModels(ModelManager* modelManager)
{
    ...
    signArchetypes.push_back(SignInternal(Physics::CShape::SIGN_STOP, modelId));

    return true;
}

std::map<Physics::CShape, const std::vector<glm::vec3>*> SignGenerator::GetModelPoints(ModelManager* modelManager)
{
...
void SignGenerator::GetRandomSignModel(unsigned int* modelId, Physics::CShape* shape) const
{

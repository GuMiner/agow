...
std::string TerrainCache::GetCacheFilePath(const vec::vec2i& subtilePos)
{
...
std::string TerrainCache::GetCacheFolder(const vec::vec2i& subtilePos)
{
...
bool TerrainCache::IsInCache(const vec::vec2i& subtilePos)
{
...
void TerrainCache::WriteVectorVector(std::ofstream* outputStream, std::vector<vec::vec3>* vector)
{
    unsigned int size = vector == nullptr ? 0 : vector->size();
    outputStream->write((char*)&size, sizeof(unsigned int));
    for (unsigned int i = 0; i < size; i++)
    {
        outputStream->write((char*)&((*vector)[i]), sizeof(vec::vec3));
    }
}

void TerrainCache::ReadVectorVector(std::ifstream* inputStream, std::vector<vec::vec3>* vector)
{
    unsigned int size;
    ...
    {
        vec::vec3 point;
        inputStream->read((char*)&point, sizeof(vec::vec3));
        vector->push_back(point);
    }
}

void TerrainCache::SaveToCache(const vec::vec2i& subtilePos, void* data)
{
    std::string cacheFileFolder = GetCacheFolder(subtilePos);
    if (_access(cacheFileFolder.c_str(), 0) != 0)
    {
...
void TerrainCache::LoadFromCache(const vec::vec2i& subtilePos, void** data)
{
    std::string cacheFilePath = GetCacheFilePath(subtilePos);
